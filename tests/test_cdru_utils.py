...
class TestCRUDUtilsGet(TestCase):
    ...
    def test_get_list_with_nonexistent_field(self):
        """Test filtering with non-existent field returns empty results."""
        request = _make_authenticated_request('GET', '/first_app/', query_params={'nonexistent': 'value'}, token=self.token)
        response = CRUDUtils.get(
            request=request,
            model_class=FirstApp,
            serializer_class=FirstAppSerializer,
    ...
    def test_get_list_with_foreignkey_lookup(self):
        ...
        request = _make_authenticated_request('GET', '/second_app/', query_params={'first_app__name': 'test1'}, token=self.token)
        response = CRUDUtils.get(
            request=request,
...
class TestCRUDUtilsPost(TestCase):
    ...
    def test_post_create_instance(self):
        ...
        request = _make_authenticated_request('POST', '/first_app/', data={
            'name': 'New Item',
...
class TestCRUDUtilsHelperMethods(TestCase):
    ...
    def test_has_middle_wildcard(self):
        ...
        self.assertTrue(CRUDUtils._has_middle_wildcard('t*t'))
        self.assertFalse(CRUDUtils._has_middle_wildcard('test*'))
        self.assertFalse(CRUDUtils._has_middle_wildcard('*test'))
        self.assertFalse(CRUDUtils._has_middle_wildcard('*test*'))

    def test_wildcard_to_regex(self):
        """Test wildcard to regex conversion."""
        regex = CRUDUtils._wildcard_to_regex('t*t')
        self.assertIn('.*', regex)

...
- `python manage.py loaddata <fixture_name>`: Loads data from a fixture file into the database.
- `python manage.py dumpdata <app_name>.<model_name>`: Exports data from the specified model into a JSON format.

### Utility Commands
- `python manage.py check`: Checks the entire Django project for potential issues without making database migrations.
- `python manage.py dbshell`: Opens the database shell for the configured database.
- `python manage.py diffsettings`: Displays the differences between the current settings and Django's default settings.
- `python manage.py inspectdb`: Generates model code by introspecting the database tables.
